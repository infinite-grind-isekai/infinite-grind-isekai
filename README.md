...
[플레이어] 스킬 선택 → 크리티컬 판정 → 데미지 계산 → 적용 → 쿨다운 등록
...
아이템을 인벤토리에서 `사용(use)` 하면 `loadout.equip(this)` 가 호출되어 장착되고,  
...
동일 슬롯 부위에 새 장비를 착용하면 기존 장비가 교체됩니다.
...
└── view/
    ...
    ├── GameMenuView.java                # 메인 메뉴 · 정보 · 인벤토리 · 상점
    ├── DungeonBattleView.java           # 전투 화면 (직업별 아스키 아트 포함)
